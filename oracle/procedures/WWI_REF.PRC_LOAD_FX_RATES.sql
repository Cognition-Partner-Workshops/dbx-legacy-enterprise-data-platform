...
IS
    ...
    TYPE t_rate_rec IS RECORD (
        from_ccy WWI_REF.FX_RATE_DAILY.FROM_CURRENCY_CD%TYPE,
        to_ccy   WWI_REF.FX_RATE_DAILY.TO_CURRENCY_CD%TYPE,
        rate_num WWI_REF.FX_RATE_DAILY.RATE_NUM%TYPE
    );
    ...
    CURSOR c_expected IS
        SELECT c.CURRENCY_CD
         ...
         WHERE NVL(c.ACTIVE_FLAG, 'Y') = 'Y'
           AND c.CURRENCY_CD <> WWI_REF.PKG_FX.c_pivot_currency;
...
BEGIN
    ...
    FOR rec IN c_expected LOOP
        SELECT COUNT(*)
         ...
         WHERE FROM_CURRENCY_CD = rec.CURRENCY_CD
           AND TO_CURRENCY_CD   = WWI_REF.PKG_FX.c_pivot_currency
        ...
        BEGIN
            ...
            WWI_REF.PKG_FX.upsert_rate(rec.CURRENCY_CD,
