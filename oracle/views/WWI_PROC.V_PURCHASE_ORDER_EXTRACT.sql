...
SELECT h.PO_ID,
       ...
       h.CURRENCY_CD,
       h.STATUS_CD,
       WWI_REF.FN_TRANSLATE_CODE('PO_STATUS', h.STATUS_CD, 'ORAERP', 'WWIDW', h.REGION_CD)
                                                        AS DW_STATUS_CD,
       h.ORDER_DT,
       h.PROMISED_DT,
       ...
       h.TOTAL_AMT,
       h.TAX_AMT,
       WWI_FIN.FN_CONVERT_AMOUNT(h.TOTAL_AMT, h.CURRENCY_CD, 'USD',
       ...
       chg.LAST_CHANGE_DT,
       CASE
           WHEN h.STATUS_CD IN ('CL', 'CA')                             THEN 'N'
  ...
  LEFT OUTER JOIN (
        SELECT pl.PO_ID,
               COUNT(*)                                                 AS LINE_COUNT,
               COUNT(CASE WHEN NVL(pl.CLOSED_FLAG, 'N') = 'N' THEN 1 END) AS OPEN_LINE_COUNT,
               SUM(pl.ORDER_QTY)                                        AS ORDERED_QTY,
               SUM(NVL(pl.RECEIVED_QTY, 0))                             AS RECEIVED_QTY,
               SUM(NVL(pl.INVOICED_QTY, 0))                             AS INVOICED_QTY
          FROM WWI_PROC.PURCHASE_ORDER_LINE pl
         GROUP BY pl.PO_ID
       ) lines
    ON lines.PO_ID = h.PO_ID
  LEFT OUTER JOIN (
        SELECT c.PO_ID, COUNT(*) AS CHANGE_COUNT, MAX(c.CHANGE_DT) AS LAST_CHANGE_DT
          FROM WWI_PROC.PO_CHANGE_ORDER c
         WHERE NVL(c.APPROVED_FLAG, 'N') = 'Y'
         GROUP BY c.PO_ID
       ) chg
